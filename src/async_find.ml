...
let stat t seen path =
  ...
  Monitor.try_with ~rest:`Raise (fun () ->
    ...
    | { kind = `Link; _ } as lstat when t.options.O.follow_links ->
      ...
      Unix.stat full_fn
      >>| (function
        (* When a symlink points to its ancestor directory,
           report only the symlink. *)
        | ({ kind = `Directory; _ } as stat) when seen_before seen stat
          ->
          Core.Std.Printf.eprintf "loop detected";
          lstat
        | stat -> stat)
    | stat -> return stat
  ) >>= (function
...
let handle_dirs t (output_fn, dir_name, stats) =
  ...
  | { kind = `Directory; _ } ->
    ...
    | None           -> maybe_visit ()
    | Some max_depth when t.current_context.depth < max_depth -> maybe_visit ()
    | Some _max_depth -> maybe_return_info ()
    end
  | _ -> return (Some info)
;;
...
let next t =
  ...
  let rec loop () =
    ...
    | `Handle current_handle ->
      ...
      | Ok ("." | "..") -> loop ()
      | Ok basename -> handle_child_or_loop (path_append t.current_context.dir_name basename)
      | Error e ->
        upon (closedir t) (fun () ->
