    ...
    | context :: rest ->
      upon
        (Monitor.try_with (fun () ->
           t.to_visit <- rest;
           let%map handle = Unix.opendir (full_path_name t context.dir_name) in
...
let filter t file =
  ...
  | None -> return None
  | Some file ->
    if t.current_context.depth < t.options.O.min_depth
    then return None
    else (
      match t.options.O.filter with
      | None -> return (Some file)
      | Some f ->
        let%map keep = f file in
        if keep then Some file else None)
;;
...
let create ?(options = Options.default) dir =
  ...
  ; options
  ; to_visit = []
  ; current_context = { dir_name = []; seen = []; depth = 0 }
...
let to_list t = fold t ~init:[] ~f:(fun acc file -> return (file :: acc)) >>| List.rev
let find_all ?options dir = to_list (create ?options dir)
